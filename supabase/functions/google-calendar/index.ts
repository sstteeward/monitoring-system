...
Deno.serve(async (request) => {
  ...
  const body = await request.json().catch(() => ({})); const action = body.action as string; const redirectUri = `${supabaseUrl}/functions/v1/google-calendar?callback=1`;
