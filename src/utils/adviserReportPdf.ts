...
function ojtSection(p: DailyReportPayload): string {
  ...
  const behind = o.students_behind.length === 0
    ? emptyRow('No student is behind expected SIL progress.')
    : `<table class="dr-table">
  ...
  return `<div class="dr-section">
    ...
    ${behind}
    <p class="dr-note">Expected hours are ${p.settings.daily_limit_minutes / 60} hours per SIL working day
...
export function buildDailyReportHtml(report: DailyReport, logo: string | null): string {
  ...
  return `<div class="dr-doc">
    <div class="dr-head">
      <div class="dr-brand">
        ...
        <div>
          ...
          <div class="dr-brand-sub">SIL Monitoring System</div>
        </div>
      </div>
      <h1 class="dr-title">Daily SIL Monitoring Report<span>Section Adviser Report</span></h1>
    </div>
    ...
    <div class="dr-foot">
      <span>Generated by the Asian College SIL Monitoring System for ${escapeHtml(adviserName)}.</span>
...
export const dailyReportFilename = (report: DailyReport): string =>
  `SIL-Daily-Report-${report.report_date}.pdf`;
...
<title>${escapeHtml(`Daily SIL Report — ${formatReportDate(report.report_date)}`)}</title>
