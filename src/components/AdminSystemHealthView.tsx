            ...
            <div className="admin-table-card">
                <div className="admin-table-header" style={{ borderBottom: '1px solid var(--admin-border)', paddingBottom: '1.5rem', marginBottom: '1.5rem' }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <div>
                            <div className="admin-table-title">System Health Monitoring</div>
                            <div style={{ color: 'var(--text-muted)', fontSize: '0.875rem' }}>Real-time metrics on database performance and active system usage.</div>
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', background: isHealthy ? 'var(--badge-student-bg)' : 'var(--badge-admin-bg)', padding: '0.5rem 1rem', borderRadius: 20, border: '1px solid var(--border)' }}>
                            <div style={{ width: 8, height: 8, borderRadius: '50%', background: isHealthy ? 'var(--success)' : 'var(--warning)', boxShadow: `0 0 8px ${isHealthy ? 'var(--success)' : 'var(--warning)'}` }} />
                            <span style={{ color: isHealthy ? 'var(--badge-student-text)' : 'var(--badge-admin-text)', fontSize: '0.85rem', fontWeight: 600, textTransform: 'uppercase' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '1.5rem', padding: '0 1.5rem 1.5rem' }}>
                    ...
                    <div style={{ background: 'var(--bg-elevated)', padding: '1.5rem', borderRadius: 12, border: '1px solid var(--admin-border)' }}>
                        ...
                        <div style={{ fontSize: '2rem', fontWeight: 700, color: 'var(--text-primary)' }}>
                            {health?.latencyMs} <span style={{ fontSize: '1rem', color: 'var(--text-muted)', fontWeight: 500 }}>ms</span>
                        </div>
