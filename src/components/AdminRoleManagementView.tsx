...
const AdminRoleManagementView: React.FC = () => {
    ...
    const handleChangeUserRole = async (user: Profile, role: 'coordinator' | 'company') => {
        const roleLabel = role === 'company' ? 'Company' : 'Coordinator';
        if (!confirm(`Are you sure you want to set ${user.first_name} ${user.last_name}'s account role to ${roleLabel}?`)) return;
        ...
        try {
            ...
            setPromoSearchQuery('');
            setPromoResults([]);
            alert(`Account updated to ${roleLabel} successfully.`);
        } catch (e: any) {
            console.error('Role update error:', e);
            alert('Failed to update user role: ' + (e?.message || 'Check logs'));
        } finally {
            setSaving(false);
        }
    };
                ...
                <div className="admin-table-card glass-card">
                    <div className="admin-table-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
                        ...
                        <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
                            ...
                            >
                                ...
                                {showPromoSearch ? 'Close' : 'Change User Role'}
                            </button>
                            <input
                                type="text"
                    ...
                    {showPromoSearch && (
                        <div style={{ padding: '1rem 1.25rem', borderBottom: '1px solid var(--admin-border)', background: 'rgba(59, 130, 246, 0.03)' }}>
                            <div style={{ fontSize: '0.8rem', color: 'var(--admin-text-secondary)', marginBottom: '0.75rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Set Account Role</div>
                                        ...
                                        <div key={p.id} className="glass-card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.6rem 1rem', borderRadius: 8, border: '1px solid var(--admin-border)' }}>
                                            ...
                                            <div style={{ display: 'flex', gap: '0.4rem' }}>
                                                <button
                                                    ...
                                                    onClick={() => handleChangeUserRole(p, 'coordinator')}
                                                    disabled={saving}
                                                >
                                                    Make Coordinator
                                                </button>
                                                <button
                                                    ...
                                                    style={{ padding: '0.3rem 0.6rem', fontSize: '0.75rem' }}
                                                    onClick={() => handleChangeUserRole(p, 'company')}
                                                    disabled={saving}
                                                >
                                                    Make Company
                                                </button>
