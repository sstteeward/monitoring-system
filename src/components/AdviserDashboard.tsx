...
const AdviserDashboard: React.FC = () => {
    ...
    const [initialApprovalsTab, setInitialApprovalsTab] = useState<'students' | 'journals' | 'documents' | 'dtr'>('students');
    ...
    /*
     * A DTR notification links to /adviser/approvals?tab=dtr, so arriving from
     * the bell (or the email) has to open that tab, not the default one.
     */
    useEffect(() => {
        if (currentView !== 'approvals') return;
        const tab = new URLSearchParams(location.search).get('tab');
        if (tab === 'students' || tab === 'journals' || tab === 'documents' || tab === 'dtr') {
            setInitialApprovalsTab(tab);
        }
    }, [currentView, location.search]);

    const navigateTo = (view: View, param?: any) => {
        let query = '';
        if ((view === 'settings' || view === 'approvals') && param) {
            query = `?tab=${param}`;
                ...
                <div className="page-content">
                    ...
                    {currentView === 'approvals' && (
                        <AdviserApprovalsView
                            // Keyed on the tab so a deep link (a DTR notification
                            // opening ?tab=dtr) lands on the right tab even when
                            // this view is already mounted.
                            key={initialApprovalsTab}
