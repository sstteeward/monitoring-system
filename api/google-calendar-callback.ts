...
export default async function handler(request: any, response: any) {
  ...
  if (!googleClientId || !googleClientSecret || !stateSecret || !supabaseUrl || !serviceRoleKey || !code || !state) return complete(response, 'error');
  ...
  if (!encodedState || !signature || signature.length !== expectedSignature.length || !timingSafeEqual(Buffer.from(signature), Buffer.from(expectedSignature))) return complete(response, 'error');

  let claim: OAuthState;
  try { claim = decodeState(encodedState); } catch { return complete(response, 'error'); }
  if (claim.exp < Date.now()) return complete(response, 'error');
  ...
  if (!tokenResponse.ok || !token?.access_token || !token?.refresh_token) return complete(response, 'error');
  ...
  if (!connectionResponse.ok) return complete(response, 'error');
